// Mock data for FlowerProject React Native - extracted from backend database
// This data will be loaded into AsyncStorage on first app load
...
// Helper function to initialize AsyncStorage with mock data
export async function initializeAsyncStorage() {
    try {
        ...
        const existingStock = await AsyncStorage.getItem('stock');
        if (!existingStock) {
            await AsyncStorage.setItem('stock', JSON.stringify(initialStock));
        }
        ...
        console.log('✅ AsyncStorage initialized with mock data');
    } catch (error) {
