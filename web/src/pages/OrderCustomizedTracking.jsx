...
const OrderCustomizedTracking = ({ user }) => {
    ...
    useEffect(() => {
        const generateTrackingSteps = (requestObj) => {
            ...
            return requestObj.deliveryMethod === 'pickup'
                ? baseSteps
                : withDeliveryFailedAttemptStep(baseSteps, requestObj);
        };
    ...
    const handleUploadReceipt = async () => {
        ...
        try {
            ...
            if (updatedRequest) {
                const updatedPaymentMetadata = resolveRequestTrackingPaymentMetadata({
                    ...updatedRequest,
                    requestData: updatedRequest.data,
                });
                setRequest(prev => ({
                    ...prev,
                    receipt_url: updatedPaymentMetadata.receiptUrl,
                    gcash_reference_number: updatedPaymentMetadata.gcashReferenceNumber || prev?.gcash_reference_number || null,
                    payment_status: updatedRequest.payment_status,
                    additional_receipts: updatedPaymentMetadata.additionalReceipts,
                }));
            }
        } catch (error) {
    ...
    const getTrackingSteps = () => {
        ...
        return request.deliveryMethod === 'pickup'
            ? baseSteps
            : withDeliveryFailedAttemptStep(baseSteps, request);
    };
                                    ...
                                    >
                                        ...
                                        <div className="timeline-content">
                                            ...
                                            <div className="timeline-info-container">
                                                ...
                                                {step.key === 'out_for_delivery' && ['out_for_delivery', DELIVERY_FAILED_ATTEMPT_STATUS, 'delivered', 'completed', 'claimed'].includes(request.status) && request.rider && (
